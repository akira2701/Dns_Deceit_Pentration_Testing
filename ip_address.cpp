...
void udp_cksum(unsigned char packet[])//包的头部地址，包含以太网

{
	...
	pip_header = (ip_pkt  *)(packet + 14);
	//TCP包
	UDP_Header *pudp_header = NULL; //TCP头指针
	psd_header *pudp_psd_header = NULL;
	pudp_header = (UDP_Header *)(packet + 14 + ((pip_header->vhl) & 15) * 4);
	attachsize = ntohs(pip_header->len) - ((pip_header->vhl) & 15) * 4;
	pudp_psd_header = (psd_header *)malloc(attachsize + sizeof(psd_header));//开辟临时空间用于计算加盐和
